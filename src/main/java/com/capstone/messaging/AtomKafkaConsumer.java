...
public class AtomKafkaConsumer {
    ...
    @Retryable(
            retryFor = {AtomProcessingException.class, Exception.class},
    ...
    public void listenAtomCreate(
        ...
        try {
            // Validate event
            validateAtomEvent(event);

            // Process the skill atom event
            AtomSnapshot processedAtom = atomSnapshotService.processAtomEvent(event);
        ...
        } catch (AtomProcessingException e) {
    ...
    private void validateAtomEvent(SkillAtomEvent event) {
        log.debug("Validating skill atom event: {}", event);

        if (event == null) {
            throw new AtomProcessingException("Skill atom event cannot be null");
        }

        if (event.getId() == null) {
            throw new AtomProcessingException("Skill atom event must contain a valid ID");
        }

        if (event.getName() == null || event.getName().trim().isEmpty()) {
            throw new AtomProcessingException("Skill atom event must contain a valid name");
        }
    ...
    public void listenAtomUpdate(
        ...
        try {
            ...
            AtomSnapshot updatedAtom = atomSnapshotService.processAtomEvent(event);
